...
<div class="main-wrap">
    ...
    <div class="row">
      <div class="col-lg-2">
        <div class="side-bar d-lg-block d-none">
          <ul>
          <li><a href="{{route('hr.create_employee')}}"><i class="material-icons">add_circle</i> Create Employee</a></li>
             <li><a href="{{route('hr.employees')}}" class="active"><i class="material-icons">person</i> Employees</a></li>
      ...
      <div class="col-lg-10 pr-0 pl-0">
        ...
        <div class="row">
          <div class="col-lg-4 col-xl-6 col-md-3">
            <h2 class="page-head">Employees</h2>
          </div>
            ...
            <div class="action-pannel">
             ...
             <li>
              <a href="create-group.html" >
                <i class="material-icons">group</i> Create Group
              </a>
            </li>
             <li>
  ...
  <div class="dropdown-menu" aria-labelledby="dropdownMenuLink">
    ...
    <a class="dropdown-item" href="#" data-toggle="modal" data-target=".bd-example-modal-sm">Inactivate</a>
...
@if($emp->count())
  @foreach($emp as $k => $emps)
    <tr>
      <!-- <th> 
        <div class="custom-checkbox">
        <input type="checkbox" id="box-1" name="select_employee" value="{{$emps->id}}">
  <label for="box-1"></label>
</div>
</th> -->
      <th scope="row">{{++$i}}</th>
  ...
  <div class="dropdown-menu" aria-labelledby="dropdownMenuLink">
    ...
    <a class="dropdown-item" href="#" data-toggle="modal" data-target=".bd-example-modal-sm-{{$emps->id}}">Inactivate</a>
    <a class="dropdown-item" href="#" data-toggle="modal" data-target=".bd-example-modal-sm1-{{$emps->id}}">Add to Group</a>
  </div>
</div></td>
    </tr>
  @endforeach
  ...
  @endif
          ...
          {!! $emp->appends(request()->query())->links() !!}
   </div>      
        </div>
        </div>
...
<!-- delete modal -->
@foreach($emp as $k => $emps)
<div class="modal fade bd-example-modal-sm-{{$emps->id}}" tabindex="-1" role="dialog" aria-labelledby="mySmallModalLabel" aria-hidden="true">
  <div class="modal-dialog modal-sm">
    <div class="modal-content p-4">
     <i class="material-icons md-light red">person_off</i>
     <span class="content">Do you really want to inactivate</span>
      <div class="button-holder">
                
             <button type="submit" class="btn btn-cancel mt-4 fils mr-3" data-dismiss="modal"><span>Cancel</span></button>
             <a href="{{route('hr.inact_employee', $emps->id)}}" class="btn btn-default mt-4 filz "><span>Inactivate</span></a>
           </div>
    </div>
  </div>
</div>
...
<!-- add to group modal -->
@foreach($emp as $k => $emps)
<div class="modal fade bd-example-modal-sm1-{{$emps->id}}" tabindex="-1" role="dialog" aria-labelledby="mySmallModalLabel" aria-hidden="true">
  <div class="modal-dialog modal-sm">
    ...
    <div class="modal-content p-4">
     ...
     <form method="POST" action="{{route('hr.group_employee', $emps->id)}}" enctype="multipart/form-data">
     	@csrf
 			<div class="form-group">
               <label >Select Group <span class="required-icon"></span></label>
               <select class="form-control w-100 custom" name="group" required>
                @if(count($emp[$k]->man)>0)
               	@foreach($emp[$k]->man as $mans)
                 <option value="{{$mans->id}}">{{$mans->group_name}}</option>
                @endforeach
                @else
      ...
      <div class="button-holder">
             <a class="btn btn-default mt-3 filz mr-3" data-dismiss="modal"><span>Cancel</span></a> 
