...
<div class="main-wrap">
    ...
    <div class="row">
      <div class="col-lg-2">
        <div class="side-bar d-lg-block d-none">
          <ul>
          <li><a href="{{route('hr.create_group')}}"><i class="material-icons">add_circle</i> Create Group</a></li>
             <li><a href="{{route('hr.groups')}}" class="active"><i class="material-icons">people</i> Groups</a></li>
          </ul>
        </div>
      </div>
  ...
  <div class="dropdown-menu" aria-labelledby="dropdownMenuLink">
    <a class="dropdown-item" href="{{route('hr.group_members', $grps->id)}}">View</a>
    ...
    <a class="dropdown-item" href="#" data-toggle="modal" data-target=".bd-example-modal-sm-{{$grps->id}}">Delete</a>
                ...
                <span><i class="material-icons icon">supervised_user_circle</i><b>{{$grp[$k]->members}} Members</b></span>
                 <a href="{{route('hr.group_members', $grps->id)}}"><h6>{{$grps->group_name}}</h6> </a>
                 <ul class="pl-0 mb-0">
                   <li><span><i class="material-icons">watch_later</i> {{date('d-m-y', strtotime($grps->created_at))}}</span></li>
                   <li><span><i class="material-icons">settings</i> {{@$grp[$k]->user->name}}</span></li>
                 </ul>
               </div>
...
<!-- delete modal -->
@foreach($grp as $k => $grps)
<div class="modal fade bd-example-modal-sm-{{$grps->id}}" tabindex="-1" role="dialog" aria-labelledby="mySmallModalLabel" aria-hidden="true">
    ...
    <div class="modal-content p-4">
     ...
     <span class="content">Do you really want to delete?</span>
      <div class="button-holder">
             ...
             <a href="{{route('hr.delete_group', $grps->id)}}" class="btn btn-default mt-4 filz "><span>Delete</span></a>
           </div>
    </div>
  </div>
</div>
