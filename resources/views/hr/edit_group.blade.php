        ...
        <div class="row">
          ...
          <div class="col-lg-8 col-xl-6">
            <div class="card mt-3 mb-3">
              ...
              <div class="form-wrap">
                <form method="POST" id="group_form" action="{{ route('hr.edit_group_store', $grp->id) }}">
                        @csrf
                  <div class="form-group">
               <label >Group Name <span class="required-icon"></span></label>
                <input class="form-control" type="text" name="group_name" value="{{$grp->group_name}}" required>
             </div>
              <div class="form-group">
               <label >Manager <span class="required-icon"></span></label>
                <select class="form-control w-100 custom" name="manager">
                  ...
                  <option value="{{$mans->id}}" @if($grp->manager == $mans->id) selected @endif>{{$mans->name}} - {{$mans->employee_id}}</option>
                  @endforeach
                </select>
             </div>
            
             <div class="button-holder">
             <a onclick="document.getElementById('group_form').reset();" class="btn btn-default mt-4 filz mr-3"><span>Clear</span></a>
